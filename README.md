...
> **푸시 알림 관리자 시스템**  
> Firebase FCM 기반 실시간 푸시 발송,  웹 관리자 페이지를 갖춘 Spring Boot 3 푸시 관리 애플리케이션
...
**푸시 관리 서버**는 Spring Boot 3으로 구축된 푸시 알림 관리 시스템입니다. Firebase FCM을 활용한 푸시 발송, JWT 기반 보안 인증, 포괄적인 발송 이력 관리 기능을 제공합니다.

### 이 프로젝트의 특별한 점

- **모듈화**: 모듈화로 인해 어떤 서비스에도 적용 가능한 Noti 관리자 페이지
- **회원 정보 동기화**: 데이터베이스에 연결해 회원 정보 동기화 버튼으로 모든 회원들에게 선택 혹은 일괄 Noti 가능
- **다중화**: 여러 서비스들의 Noti 알림 관리를 한곳에서 전송 및 관리 가능
- **보안**: JWT 토큰 기반 인증 + 리프레시 토큰 구현
...
### 핵심 푸시 관리
- **모듈화**로 대부분의 Spring 프로젝트에 접목 가능
- **다중화** 작업으로 한개의 관리자 페이지에서 여러 프로젝트 Notification 지원
...
4. **모듈화**
- 다양한 Spring Boot 기반 서비스에 연동 가능
- 서비스 연결시 회원 정보 동기화 로직

5. **다중화**
- 동시 여러 서비스 Notification 전송 관리 기능
- 모바일 ,웹등 서비스 별 Noti 전송 가능
